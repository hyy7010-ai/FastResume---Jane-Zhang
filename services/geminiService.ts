...
export const analyzeResume = async (
  ...
  const systemInstruction = `
    Role: Senior Executive Headhunter & CV Analyst.
    Task: Deeply analyze the candidate's resume against the Target Job Description (JD).
    
    *** CRITICAL EXTRACTION RULES (MISSING NOTHING) ***
    1. **PROFESSIONAL EXPERIENCE**: Extract ALL paid roles, internships, and freelance work found in the resume. 
       - Do not skip older roles. Do not skip unrelated roles (e.g. "Barista").
    2. **VOLUNTEER & LEADERSHIP**: Extract ALL community service, student club roles, mentorships, and volunteer work.
       - Map these to the "volunteer" array.
    3. **PROJECTS**: Extract ALL academic projects, hackathons, personal portfolios, and case studies.
       - Map these to the "schoolProjects" array.
    4. **REFERENCES**: Extract ALL references listed (Name, Title, Contact).
    
    *** MATCHING LOGIC ("isMatch") - STRICT SEPARATION ***
    For every single item in Experience, Volunteer, and Projects, you MUST set "isMatch" based on RELEVANCE:
    
    - **isMatch: true**: 
      The role is DIRECTLY RELEVANT to the JD. It involves similar skills, tools, industry, or transferrable soft skills (Leadership/Management) required by the target job.
      
    - **isMatch: false**: 
      The role is UNRELATED (e.g., "Waiter" for a "Coding" job, "Retail Assistant" for "Accounting").
      NOTE: You MUST extract these roles, but mark them as false so they can be hidden in the "Other History" section. Do not delete them.
